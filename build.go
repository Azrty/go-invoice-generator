...
func (doc *Document) appendTotal() {
	...
	for _, item := range doc.Items {
		for _, tax := range item.Taxes {
			...
			if taxType == TaxTypeAmount {
				...
				tax._total = tax._total.Add(item.TotalWithoutTaxAndWithDiscount().Mul(taxAmount.Div(divider)))
	...
	for _, tax := range UniqueTaxes {
		// Draw tax title
		doc.pdf.SetY(doc.pdf.GetY() + taxY)
		doc.pdf.SetX(120)
		doc.pdf.SetFillColor(doc.Options.DarkBgColor[0], doc.Options.DarkBgColor[1], doc.Options.DarkBgColor[2])
		doc.pdf.Rect(120, doc.pdf.GetY(), 40, 10, "F")
		...
		doc.pdf.CellFormat(
			40,
			10,
			doc.encodeString(doc.ac.FormatMoneyDecimal(tax._total)),
			"0",
			0,
			"L",
			false,
